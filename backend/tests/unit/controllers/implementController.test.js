...
describe("implementController", () => {
  ...
  describe("updateImplement()", () => {
    ...
    test("al reemplazar la imagen llama a deleteFromGCS con la URL completa existing.image_url (no con la ruta relativa)", async () => {
      const OLD_URL = '/uploads/implements/old-seed.jpg';
      const NEW_URL = '/uploads/implements/new-seed.jpg';

      const req = createMockReq({ id: '1' }, { image_url: NEW_URL });
      const res = createMockRes();
      const next = createMockNext();

      mockFindById.mockResolvedValue({ ...mockImplement, image_url: OLD_URL });
      mockUpdate.mockResolvedValue({ ...mockImplement, image_url: NEW_URL });
      // Simula el comportamiento del extractor: devuelve la ruta RELATIVA
      // (implements/old-seed.jpg). El bug anterior pasaba ese valor a
      ...
      mockExtractGCSPath.mockReturnValue('implements/old-seed.jpg');
