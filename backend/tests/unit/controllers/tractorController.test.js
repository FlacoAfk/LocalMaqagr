...
// Mock de storage (upload/delete/extract): por defecto extractGCSPath
// devuelve null para no activar el flujo de borrado en tests heredados.
const mockUploadToGCS = jest.fn();
const mockDeleteFromGCS = jest.fn();
const mockExtractGCSPath = jest.fn(() => null);
jest.unstable_mockModule("../../../src/config/storage.js", () => ({
  uploadToGCS: mockUploadToGCS,
  deleteFromGCS: mockDeleteFromGCS,
  extractGCSPath: mockExtractGCSPath,
  __esModule: true,
}));
...
describe("tractorController", () => {
  beforeEach(() => {
    ...
    mockExtractGCSPath.mockReturnValue(null);
    mockDeleteFromGCS.mockReset();
  ...
  describe("updateTractor()", () => {
    ...
    test('envía image_url null cuando el usuario la elimina', async () => {
      const req = createMockReq({ id: '1' }, { image_url: null });
      const res = createMockRes();
      const next = createMockNext();

      mockFindById.mockResolvedValue({ ...mockTractor, image_url: '/uploads/tractors/old.jpg' });
      mockUpdate.mockResolvedValue({ ...mockTractor, image_url: null });

      await callHandler(updateTractor, req, res, next);

      expect(mockUpdate.mock.calls[0][1]).toHaveProperty('image_url', null);
    });

    test('omite image_url cuando no forma parte del update', async () => {
      const req = createMockReq({ id: '1' }, { status: 'maintenance' });
      const res = createMockRes();
      const next = createMockNext();

      mockFindById.mockResolvedValue({ ...mockTractor, image_url: '/uploads/tractors/old.jpg' });
      mockUpdate.mockResolvedValue({ ...mockTractor, status: 'maintenance' });

      await callHandler(updateTractor, req, res, next);

      expect(mockUpdate.mock.calls[0][1]).not.toHaveProperty('image_url');
    });

    test('al reemplazar la imagen llama a deleteFromGCS con la URL completa existing.image_url (no con la ruta relativa)', async () => {
      const OLD_URL = '/uploads/tractors/old-seed.jpg';
      const NEW_URL = '/uploads/tractors/new-seed.jpg';

      const req = createMockReq({ id: '1' }, { image_url: NEW_URL });
      const res = createMockRes();
      const next = createMockNext();

      mockFindById.mockResolvedValue({ ...mockTractor, image_url: OLD_URL });
      mockUpdate.mockResolvedValue({ ...mockTractor, image_url: NEW_URL });
      // Simula el comportamiento del extractor: devuelve la ruta RELATIVA
      // (tractors/old-seed.jpg). El bug anterior pasaba ese valor a
      // deleteFromGCS y era rechazado por INVALID_PATH. El fix debe pasar
      // la URL completa (OLD_URL) para que deleteFromGCS la resuelva bien.
      mockExtractGCSPath.mockReturnValue('tractors/old-seed.jpg');
      mockDeleteFromGCS.mockResolvedValue({ ok: true, code: 'DELETED' });

      await callHandler(updateTractor, req, res, next);

      expect(mockDeleteFromGCS).toHaveBeenCalledTimes(1);
      expect(mockDeleteFromGCS).toHaveBeenCalledWith(OLD_URL);
    });
  });
