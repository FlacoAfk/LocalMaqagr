...
const uploadMiddleware = jest.fn((req, _res, next) => next?.());
...
describe("tractor.routes duplicate cleanup", () => {
  ...
  test("mutation routes are admin-protected and keep cache invalidation", async () => {
    ...
    expect(postRoute.route.stack[2].handle).toBe(uploadMiddleware);
    ...
    expect(putRoute.route.stack[2].handle).toBe(uploadMiddleware);
